...
Examples:
  PORTRAIT_VAM1_PASS='…' python3 portrait-login.py vam1
  PORTRAIT_CLIENT_CMD='wine /path/to/Imagine.exe' \\
    PORTRAIT_VAM1_PASS='…' python3 portrait-login.py vam1 --launch
  python3 portrait-login.py vam1 --no-launch   # game already open
  # preferred: ./studio (menu) or portrait-orch.py up
"""
...
def reload_login_config() -> None:
    """Re-read PORTRAIT_* knobs after website tunables / .env load."""
    global WINDOW_TITLE, CLIENT_CMD
...
def snap(role: str, step: str, wid: str) -> None:
    try:
        from portrait_common import debug_snap

        debug_snap(role, step, wid)
    except Exception as e:
        print(f"warning: debug-snap {role}/{step}: {e}", file=sys.stderr)
    ...
    *,
    ...
    try:
        ...
        if not credentials_only:
            ...
            snap(role, "after-start-game", wid)
            print(
...
def main() -> None:
    sys.path.insert(0, str(Path(__file__).resolve().parent))
    from portrait_common import apply_login_tunables_from_website, load_portrait_env
    ...
    if args.start_only:
        ...
        try:
            ...
            snap(role, "start-game-failure", wid)
            raise
        return
