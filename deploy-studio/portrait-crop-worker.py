...
Examples:
  python3 portrait-crop-worker.py crop ~/va_screenshots/Screenshot_….png
  python3 portrait-crop-worker.py watch
  ./studio   # preferred entry for worker/orch
...
from portrait_common import ROOT, WORK_DIR  # noqa: E402

DEFAULT_INBOX = Path("/home/cat/repos/smt/va_screenshots")
DEFAULT_OUT = (
    ROOT / "work" / "portrait-crops"
    if (ROOT / "website").is_dir()
    else WORK_DIR.parent / "portrait-crops"
)
...
# Derived 2026-08-14 from Screenshot_20260814_221258.png (1440×900) vs
# Screenshot_20260814_221258_cropped.png (577×792): exact subimage match,
# MAD=0 at LTRB=447,103,1024,895. Fractions so high-res headless works.
STUDIO_FRAC = (
    0.310417,  # left
    0.114444,  # top
    0.711111,  # right
...
def studio_box(w: int, h: int) -> Box:
    """Resolution-independent armory portrait window (post camera-init)."""
    l, t, r, b = STUDIO_FRAC
    return Box(
        int(round(w * l)),
        int(round(h * t)),
...
def variants_for(im: Image.Image) -> list[tuple[str, Box]]:
    ...
    # Preferred: fractional studio crop (camera-init framing).
    out.append(("studio", studio_box(w, h)))
