...
Env (deploy-studio/.env via dotenv):
...
from portrait_common import (  # noqa: E402
    ...
    ROOT,
    ALLOWED_MANNEQUIN_ROLES,
    MAX_MANNEQUIN_CLIENTS,
    WINDOWS_PATH,
    WINDOW_TITLE,
    apply_login_tunables_from_website,
...
AFTER_LAUNCH_TIMEOUT = 90.0
BETWEEN_CLIENTS_SEC = 3.0
BETWEEN_CAM_SEC = 2.5
CAM_FOCUS_SEC = 1.2
LOGIN_RETRIES = 3
ONLINE_TIMEOUT = 60.0
...
def reload_orch_config() -> None:
    """Re-read orch timing knobs after website tunables / .env load."""
    global AFTER_LAUNCH_TIMEOUT, BETWEEN_CLIENTS_SEC, BETWEEN_CAM_SEC
    global CAM_FOCUS_SEC, LOGIN_RETRIES, ONLINE_TIMEOUT, CHAR_SELECT_SEC
...
def run_login_step(role: str, wid: str, *extra: str) -> None:
    ...
    if not os.environ.get(pass_key, "").strip() and not os.environ.get(
        ...
        die(f"set {pass_key} in deploy-studio/.env (or env) before orch up")
    env = os.environ.copy()
    ...
    *,
    ...
    for attempt in range(1, max(1, retries) + 1):
        ...
        try:
            ...
            debug_snap(role, "credentials-failed", wid)
            continue
        ...
        try:
            ...
            debug_snap(role, "start-failed", wid)
            continue
        if not studio_ok:
            print(
...
def cmd_up(args: argparse.Namespace) -> None:
    ...
    for i, role in enumerate(roles):
        if not args.reuse_windows:
            ...
            # Per-launch cap: never exceed MAX even mid-run.
            live_n = count_imagine_clients()["count"]
            if live_n >= MAX_MANNEQUIN_CLIENTS and not args.ok_existing:
                die(
                    f"already at {live_n}/{MAX_MANNEQUIN_CLIENTS} Imagine "
                    "client(s) before launching next role — aborting"
                )
...
def main() -> None:
    ...
    p_up.add_argument(
        ...
        action="store_true",
        help=(
            "DANGEROUS: allow launching even when Imagine clients already run "
            f"(can exceed the {MAX_MANNEQUIN_CLIENTS}-client cap). Prefer orch down."
        ),
    )
