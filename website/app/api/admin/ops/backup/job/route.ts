...
import { getOpsBackupJob } from "@/lib/ops-sidecar"
...
export async function GET(request: Request) {
  ...
  try {
    const job = await getOpsBackupJob(jobId, session.username)
    if (!job.ok && job.error === "unauthorized") {
      return apiFail("Ops token rejected by sidecar", 502, "OPS")
    }
    if (!job.ok && job.error === "unknown_job") {
      return apiFail("Unknown backup job", 404, "NOT_FOUND")
    }
    return apiOk(job)
  } catch (error) {
    return apiFail(
      error instanceof Error ? error.message : "Backup job lookup failed",
      502,
      "OPS"
    )
  }
