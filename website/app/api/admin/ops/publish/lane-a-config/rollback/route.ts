...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!result.ok) {
      const msg =
        result.error === "not_allowed"
          ? "Ops sidecar missing Lane A config rollback verb. Restart pnpm run ops-sidecar."
          : result.detail ||
            result.error ||
            result.errors?.join("; ") ||
    ...
    return apiOk(result, result.message || "Config rolled back")
  } catch (error) {
    return apiFail(
      error instanceof Error ? error.message : "Config rollback failed",
      502,
      "OPS"
    )
  }
